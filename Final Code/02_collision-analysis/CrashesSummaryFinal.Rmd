...
# Define IOD as 1 if there is an injury or a death
...
model_age <- glm(IOD ~ poly(DRIVER_AGE_1,2) + poly(DRIVER_AGE_2,2) + DRIVER_AGE_1:DRIVER_AGE_2,family=binomial(link='logit'),data=df)
...
df$VEHICLE_WEIGHT_DIFFERENTIAL <- abs(df$VEHICLE_WEIGHT_1 - df$VEHICLE_WEIGHT_2)

model_weight <- glm(IOD ~ log(VEHICLE_WEIGHT_1) + log(VEHICLE_WEIGHT_2) + VEHICLE_WEIGHT_DIFFERENTIAL ,family=binomial(link='logit'),data=df)
...
Vehicle Weight was calculated through the extremely scientific process of asking Google how much a particular type of vehicle weighs. There is a clear relationship between the weight of a vehicle and the risk of injury in an accident, and the relationship becomes stronger when you take the log of the weight. I was hoping to find a strong relationship between the weight differential between two vehicles but the coefficient is small, so I left it out of the final model. More detailed work will need to be done to determine the impact of weight differential on collision severity.  
